...
from re import A
import sys, time
from tokenize import Name   # sys needed to access cmd line args and sys.exit()
...
def simplestmt():
   if(token.category == 3):
      assignmentstmt()
   elif(token.category == 1):
      printstmt()

def assignmentstmt():
   consume(NAME)
   consume(ASSIGNOP)
   if(token.category == 10):
       print('\nError on '+ "' '" + ' line ' +
         str(token.line) + ' column ' + str(token.column))
   expr()
...
def expr():
   while(token.category == 8 or token.category == 7 or token.category == 2 or token.category == 3 or token.category == 9):
      term()
...
def term():
   if(token.category == 8):
      factor()
   elif(token.category == 7):
      factor()
   elif(token.category == 2):
      factor()
   elif(token.category == 3):
      factor()
   elif(token.category == 5):
      factor()
   elif(token.category == 6):
      factor()
   elif(token.category == 9):
      factor()
